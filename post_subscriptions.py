...
def test_check_status_code_equals_400():
    json_body["filteringCriteria"]["islocal"] = ""
    response = requests.post("http://127.0.0.1:8080/mec_service_mgmt/v1/applications/12321/subscriptions",
                             json = json_body)
...
def test_check_status_code_equals_404():
    response = requests.post("http://127.0.0.1:8080/mec_service_mgmt/v1/applications/12321/subscriptionsxxx")
...
# Reload the json to test the error HTTP code 403
with open("Subscription_Example.json", "r") as read_content:
    json_body_ = json.load(read_content)
